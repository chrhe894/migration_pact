---
tags:
  - återvändande
  - koncept
---


# CON-RET-001

## Begrepp

Return border procedure

---

## Definition

Ett gränsförfarande för återvändande som tillämpas på tredjelandsmedborgare vars asylansökan avslagits i asylgränsförfarandet. Personen tillåts inte resa in på territoriet utan ska återvända direkt.

---

## Syfte

Säkerställer ett snabbt och effektivt verkställande av avslagsbeslut fattade i gränsförfarandet utan att personen tillåts resa in på territoriet.

---

## Skapas av

Utlöses automatiskt av ett avslagsbeslut i asylgränsförfarandet ([PROC-RET-001](../processes/return-border-procedure.md)).

---

## Rättslig grund

[Return Border Procedure artikel 4](../articles/return-004.md)

---

## Relaterade begrepp

- [CON-BRD-001 Border procedure](../../border-procedure/concepts/border-procedure.md)
- [CON-RET-002 Voluntary departure](voluntary-departure.md)

---

## Används i

- [ART-RET-004](../articles/return-004.md)
- [PROC-RET-001 Return border procedure](../processes/return-border-procedure.md)
