
# Solidaritet

## Syfte

Domänen beskriver solidaritetsmekanismen i AMMR (del IV), inklusive den årliga solidaritetspoolen, omfördelning, ansvarskompensationer och ekonomiska bidrag.

Solidaritetsmekanismen syftar till att fördela ansvaret rättvist mellan medlemsstaterna vid migrationstryck.

---

## Plats i livscykeln

| | |
|---|---|
| **Föregående** | [Ansvar](../responsibility/README.md) (migrationstryck konstateras) |
| **Nästa** | [Registrering](../registration/README.md) i mottagande stat (vid omfördelning) |
| **Inträde** | Migrationstryck eller betydande migrationssituation konstaterad |
| **Utträde** | Solidaritetsbidrag genomfört (omfördelning, kompensation eller bidrag) |

---

## Primära rättskällor

- [AMMR artikel 56](articles/ammr-056.md) — Den årliga solidaritetspoolen
- [AMMR artikel 57](articles/ammr-057.md) — Rådets genomförandeakt om solidaritetspoolen
- [AMMR artikel 63](articles/ammr-063.md) — Ansvarskompensationer
- [AMMR artikel 64](articles/ammr-064.md) — Ekonomiska bidrag
- [AMMR artikel 65](articles/ammr-065.md) — Alternativa solidaritetsåtgärder
- [AMMR artikel 66](articles/ammr-066.md) — Referensnyckel
- [AMMR artikel 67](articles/ammr-067.md) — Förfarande inför omfördelning
- [AMMR artikel 68](articles/ammr-068.md) — Förfarande efter omfördelning

---

## Processer

| Process | Beskrivning |
|---------|-------------|
| [PROC-SOL-001 Omfördelning](processes/relocation.md) | Omfördelning av sökande från en utsatt stat till en bidragande stat |

---

## Juridiska milstolpar

```text
Migrationstryck konstateras
        │
        ▼
Solidaritetspoolen aktiveras (art. 57)
        │
        ├── Omfördelning (art. 67)
        ├── Ansvarskompensationer (art. 63)
        └── Ekonomiska bidrag (art. 64)
        │
        ▼
Omfördelning genomförs
        │
        ▼
Ansvar övergår till mottagande stat (art. 68)
```

---

## Begrepp

| ID | Begrepp |
|----|---------|
| [CON-SOL-001](concepts/solidarity-pool.md) | Solidarity pool |
| [CON-SOL-002](concepts/relocation.md) | Omfördelning |
| [CON-SOL-003](concepts/responsibility-offset.md) | Responsibility offset |
| [CON-SOL-004](concepts/reference-key.md) | Reference key |

---

## Regler

### AMMR artikel 57 — Solidaritetspoolen

| Regel | Beskrivning |
|-------|-------------|
| [RULE-AMMR-057-001](rules/rule-ammr-057-001.md) | Skyldighet att bidra till solidaritetspoolen |

### AMMR artikel 63 — Ansvarskompensationer

| Regel | Beskrivning |
|-------|-------------|
| [RULE-AMMR-063-001](rules/rule-ammr-063-001.md) | Obligatorisk ansvarskompensation vid vägran att omfördela |

### AMMR artikel 66 — Referensnyckel

| Regel | Beskrivning |
|-------|-------------|
| [RULE-AMMR-066-001](rules/rule-ammr-066-001.md) | Beräkning av bidragsandel |

### AMMR artikel 67 — Omfördelning

| Regel | Beskrivning |
|-------|-------------|
| [RULE-AMMR-067-001](rules/rule-ammr-067-001.md) | Förfarande inför omfördelning |

---

## Tolkningar

| Fil | Frågeställning |
|-----|----------------|
| [solidarity-types](interpretations/solidarity-types.md) | Vilka typer av solidaritetsbidrag finns och hur förhåller de sig till varandra? |
| [relocation-flow](interpretations/relocation-flow.md) | Hur ser hela omfördelningsflödet ut från identifiering till ansvarsövergång? |

---

## Öppna frågor

| Fil | Fråga |
|-----|-------|
| [refusal-to-relocate](open_questions/refusal-to-relocate.md) | Vad händer om en stat vägrar att omfördela? |

---

## Delade moduler

| Modul | Länk |
|-------|------|
| Time limits | [shared/time-limits](../../shared/time-limits/README.md) |
| Children | [shared/children](../../shared/children/README.md) |
