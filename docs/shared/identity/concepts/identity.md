---
tags:
  - shared
  - identity
---

# CON-ID-001

## Begrepp

Identity

---

## Definition

Sökandens personidentitet, inbegripet namn, födelsedatum, födelseort, kön, medborgarskap eller statslöshet.

---

## Syfte

Utgör grundläggande information som krävs för att koppla personen till databaserade register, genomföra säkerhetskontroller och säkerställa korrekt handläggning genom hela förfarandet.

---

## Skapas av

Fastställs eller bedöms av den behöriga myndigheten vid screening och registrering utifrån personens egna uppgifter och tillgängliga handlingar ([PROC-SCR-001](../../../domains/screening/processes/screening-at-external-border.md)).

---

## Rättslig grund

[APR artikel 27](../../../domains/registration/articles/apr-027.md), punkt 1 a

---

## Kommentar

Identitet ska fastställas eller bedömas i den utsträckning som krävs enligt tillämplig unionsrätt. Kravet är inte absolut — identiteten ska klaras i den utsträckning som är relevant för förfarandet.

---

## Relaterade begrepp

- [CON-ID-002 Identity document](identity-document.md)
- [CON-REG-002 Applicant](../../../domains/registration/concepts/applicant.md)
- [CON-SCR-001 Screening](../../../domains/screening/concepts/screening.md)

---

## Används i

- [ACT-ID-001 Verify identity](../activities/verify-identity.md)
- [PROC-REG-001 Registration of an application](../../../domains/registration/processes/registration-of-an-application.md)
- [PROC-SCR-001 Screening vid yttre gräns](../../../domains/screening/processes/screening-at-external-border.md)
- [PROC-SCR-002 Screening inom territoriet](../../../domains/screening/processes/screening-within-territory.md)
