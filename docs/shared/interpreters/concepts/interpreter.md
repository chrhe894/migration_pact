# CON-INT-001

## Begrepp

Interpreter

---

## Definition

En person som tillhandahåller tolktjänster för att möjliggöra kommunikation mellan sökanden och den behöriga myndigheten under förfarandet för internationellt skydd.

---

## Syfte

Säkerställer att sökanden kan kommunicera effektivt med myndigheterna, vilket är en förutsättning för ett rättssäkert förfarande.

---

## Skapas av

Den behöriga myndigheten genom att bedöma behovet av tolktjänster och anlita en kvalificerad tolk ([ACT-INT-001](../activities/determine-interpreter-needs.md)).

---

## Rättslig grund

APR artikel 8.3

---

## Kommentar

Kostnaderna för tolkning ska täckas med offentliga medel. Tolktjänster ska tillhandahållas vid registrering, inlämnande och i tillämpliga fall vid den personliga intervjun, när ändamålsenlig kommunikation inte kan säkerställas utan sådana tjänster.

---

## Relaterade begrepp

- [CON-REG-002 Applicant](../../../domains/registration/concepts/applicant.md)

---

## Används i

- [ACT-INT-001 Determine interpreter needs](../activities/determine-interpreter-needs.md)
- [PROC-REG-001 Registration of an application](../../../domains/registration/processes/registration-of-an-application.md)
- [PROC-REG-002 Lodging an application](../../../domains/registration/processes/lodging-an-application.md)
