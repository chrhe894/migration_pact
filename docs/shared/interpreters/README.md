# Delat — Tolkar

## Syfte

Denna shared-modul dokumenterar rätten till tolktjänster under förfarandet för internationellt skydd.

Rätten till tolk är en grundläggande garanti som gäller i flera förfaranden och domäner.

---

## Primära rättskällor

- [APR artikel 8.3](../articles/apr-008.md) — Rätt till tolkning vid registrering, inlämnande och personlig intervju
- [APR artikel 30.2](articles/apr-030.md) — Tolktjänster i förvarsanläggningar och vid gränsövergångsställen

---

## Artiklar

- [ART-APR-008](articles/apr-008.md) — Rätt till tolkning och översättning
- [ART-APR-030](articles/apr-030.md) — Personlig intervju — tolkningskrav

---

## Aktiviteter

- [determine-interpreter-needs (ACT-INT-001)](activities/determine-interpreter-needs.md)

---

## Begrepp

- [Interpreter (CON-INT-001)](concepts/interpreter.md)

---

## Regler

- [RULE-INT-001](rules/rule-int-001.md) — Rätt till tolkning under det administrativa förfarandet
- [RULE-INT-002](rules/rule-int-002.md) — Kostnader för tolkning ska bäras av det allmänna

---

## Krav

- [REQ-INT-001](requirements/req-int-001.md) — Tolk ska tillhandahållas vid registrering, inlämnande och intervju
- [REQ-INT-002](requirements/req-int-002.md) — Kostnader för tolkning ska bäras av det allmänna
- [REQ-INT-003](requirements/req-int-003.md) — Tolken ska vara kvalificerad och neutral

Se [Krav — Interpreters](requirements/README.md) för fullständig kravlista.

---

## Används av

- [PROC-REG-001 Registrering av en ansökan](../../domains/registration/processes/registration-of-an-application.md)
- [PROC-REG-002 Ingivande av en ansökan](../../domains/registration/processes/lodging-an-application.md)
- [PROC-SCR-001 Screening vid yttre gräns](../../domains/screening/processes/screening-at-external-border.md)
- [PROC-ASY-001 Pröva en ansökan](../../domains/asylum-procedure/processes/examine-an-application.md)
