# Delat — Dokument

## Syfte

Denna shared-modul dokumenterar handlingar som utfärdas till sökande under förfarandet för internationellt skydd.

---

## Primära rättskällor

- [APR artikel 29](../../domains/registration/articles/apr-029.md) — Handlingar som tillhandahålls sökanden

---

## Begrepp

- [Registration certificate (CON-REG-006)](../../domains/registration/concepts/registration-certificate.md)
- [Applicant document (CON-REG-007)](../../domains/registration/concepts/applicant-document.md)

---

## Regler

- [RULE-APR-029-001](../../domains/registration/rules/rule-apr-029-001.md) — Skyldighet att tillhandahålla registreringshandling
- [RULE-APR-029-002](../../domains/registration/rules/rule-apr-029-002.md) — Skyldighet att utfärda sökandehandling
- [RULE-APR-029-003](../../domains/registration/rules/rule-apr-029-003.md) — Giltighetstid för sökandehandling

---

## Används av

- [PROC-REG-003 Issue applicant documents](../../domains/registration/processes/issue-applicant-documents.md)
