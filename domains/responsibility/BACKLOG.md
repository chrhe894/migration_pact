<div align="right">← <a href="README.md">Responsibility</a></div>

# Responsibility Backlog

## Domänöversikt

- [x] README
- [x] Identifiera centrala begrepp
- [x] Identifiera processer
- [x] Identifiera beroenden till Shared

---

## Begrepp

- [x] Responsible Member State — CON-RES-001
- [x] Criteria hierarchy — CON-RES-002
- [x] Take charge request — CON-RES-003
- [x] Take back notification — CON-RES-004
- [ ] Transfer — överföring av sökande till ansvarig stat
- [ ] Determining Member State — den stat som genomför ansvarsbestämningen

---

## Artiklar — Kriterier (kapitel II)

- [x] AMMR artikel 24 — Kriteriernas rangordning
- [x] AMMR artikel 25 — Ensamkommande barn
- [x] AMMR artikel 26 — Familjemedlem lagligen bosatt
- [x] AMMR artikel 27 — Familjemedlem med pågående ansökan
- [x] AMMR artikel 28 — Förfarande rörande familjer
- [x] AMMR artikel 29 — Uppehållshandlingar eller viseringar
- [x] AMMR artikel 30 — Examensbevis
- [x] AMMR artikel 31 — Inresa efter upphävt viseringskrav
- [x] AMMR artikel 32 — Ansökan i flygplatstransit
- [x] AMMR artikel 33 — Inresa (irreguljär gränspassage)

---

## Artiklar — Övrigt

- [x] AMMR artikel 34 — Personer i beroendeställning
- [x] AMMR artikel 35 — Diskretionär bedömning
- [x] AMMR artikel 36 — Den ansvariga statens skyldigheter
- [x] AMMR artikel 37 — Ansvarets upphörande
- [x] AMMR artikel 38 — Inledande av förfarandet
- [x] AMMR artikel 39 — Framställan om övertagande
- [x] AMMR artikel 40 — Svar på framställan om övertagande
- [x] AMMR artikel 41 — Avisering om återtagande
- [x] AMMR artikel 42 — Överföringsbeslut
- [x] AMMR artikel 43 — Rättsmedel
- [x] AMMR artikel 46 — Överföring
- [ ] AMMR artiklarna 44–45 — Suspensiv verkan och garantier

---

## Regler

- [x] RULE-AMMR-024-001 — Kriterierna i angiven ordning
- [x] RULE-AMMR-024-002 — Situationen vid första registrering
- [x] RULE-AMMR-025-001 — Ensamkommande barn
- [x] RULE-AMMR-026-001 — Familjemedlem lagligen bosatt
- [x] RULE-AMMR-027-001 — Familjemedlem med pågående ansökan
- [x] RULE-AMMR-028-001 — Familjer ska inte separeras
- [x] RULE-AMMR-029-001 — Uppehållshandlingar eller viseringar
- [x] RULE-AMMR-030-001 — Examensbevis
- [x] RULE-AMMR-031-001 — Viseringsfri inresa
- [x] RULE-AMMR-032-001 — Flygplatstransit
- [x] RULE-AMMR-033-001 — Irreguljär inresa
- [x] RULE-AMMR-033-002 — Tidsbegränsning av ansvar
- [x] RULE-AMMR-035-001 — Diskretionär bedömning
- [x] RULE-AMMR-034-001 — Sammanförande av beroendesökande
- [x] RULE-AMMR-036-001 — Skyldighet att överta och återta
- [x] RULE-AMMR-037-001 — Ansvarets upphörande
- [x] RULE-AMMR-038-001 — Inleda förfarandet utan dröjsmål
- [x] RULE-AMMR-039-001 — Tidsfrist för framställan om övertagande
- [x] RULE-AMMR-040-001 — Tidsfrist för svar och tyst godkännande
- [x] RULE-AMMR-041-001 — Avisering om återtagande
- [x] RULE-AMMR-042-001 — Överföringsbeslut
- [x] RULE-AMMR-043-001 — Rätt till rättsmedel
- [x] RULE-AMMR-046-001 — Tidsfrist för överföring

---

## Processer

- [x] PROC-RES-001 — Determine responsible Member State

---

## Diagram

- [x] Determine responsible Member State

---

## Shared dependencies

- [x] children
- [x] identity
- [x] time-limits
- [ ] documents

---

## Tolkningar

- [x] Registreringstidpunkten som referenspunkt

---

## Öppna frågor

- [x] Ansvarets upphörande
- [ ] Hur hanteras situationer där inget kriterium ger en ansvarig stat?
