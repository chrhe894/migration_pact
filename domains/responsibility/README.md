<div align="right">← <a href="../../README.md">Kunskapsbasen</a></div>

# Responsibility

## Syfte

Domänen beskriver reglerna för att fastställa vilken medlemsstat som är ansvarig för att pröva en ansökan om internationellt skydd, enligt Asylum and Migration Management Regulation (AMMR).

Ansvarsbestämningen är central i det gemensamma europeiska asylsystemet — den avgör i vilken medlemsstat sökanden ska få sin ansökan prövad.

---

## Primära rättskällor

- [AMMR artikel 24](articles/ammr-024.md) — Kriteriernas rangordning
- [AMMR artikel 25](articles/ammr-025.md) — Ensamkommande barn
- [AMMR artikel 26](articles/ammr-026.md) — Familjemedlemmar som är lagligen bosatta
- [AMMR artikel 27](articles/ammr-027.md) — Familjemedlemmar som har ansökt
- [AMMR artikel 28](articles/ammr-028.md) — Förfarande rörande familjer
- [AMMR artikel 29](articles/ammr-029.md) — Uppehållshandlingar eller viseringar
- [AMMR artikel 30](articles/ammr-030.md) — Examensbevis eller kvalifikationer
- [AMMR artikel 31](articles/ammr-031.md) — Inresa efter upphävt viseringskrav
- [AMMR artikel 32](articles/ammr-032.md) — Ansökan inom transitområdet på flygplats
- [AMMR artikel 33](articles/ammr-033.md) — Inresa (irreguljär gränspassage)
- [AMMR artikel 35](articles/ammr-035.md) — Diskretionär bedömning

---

## Processer

| Process | Beskrivning |
|---------|-------------|
| [PROC-RES-001 Determine responsible Member State](processes/determine-responsible-member-state.md) | Fastställ vilken medlemsstat som ansvarar för prövningen |

---

## Kriteriernas rangordning

Kriterierna tillämpas i följande ordning (artikel 24). Om ett kriterium ger en ansvarig stat, tillämpas inte de efterföljande:

```text
1. Ensamkommande barn (art. 25)
2. Familjemedlem lagligen bosatt (art. 26)
3. Familjemedlem med pågående ansökan (art. 27)
4. Familjeförfarande (art. 28)
5. Uppehållshandling eller visering (art. 29)
6. Examensbevis eller kvalifikation (art. 30)
7. Inresa efter upphävt viseringskrav (art. 31)
8. Ansökan i transitzon på flygplats (art. 32)
9. Irreguljär inresa / landsättning (art. 33)
```

Om inget kriterium ger en ansvarig stat: den medlemsstat där ansökan först registrerades.

---

## Begrepp

| ID | Begrepp |
|----|---------|
| [CON-RES-001](concepts/responsible-member-state.md) | Responsible Member State |
| [CON-RES-002](concepts/criteria-hierarchy.md) | Criteria hierarchy |
| [CON-RES-003](concepts/take-charge-request.md) | Take charge request |
| [CON-RES-004](concepts/take-back-notification.md) | Take back notification |

---

## Regler

### AMMR artikel 24 — Rangordning

| Regel | Beskrivning |
|-------|-------------|
| [RULE-AMMR-024-001](rules/rule-ammr-024-001.md) | Kriterierna tillämpas i angiven ordning |
| [RULE-AMMR-024-002](rules/rule-ammr-024-002.md) | Ansvar fastställs baserat på situationen vid första registrering |

### AMMR artikel 25 — Ensamkommande barn

| Regel | Beskrivning |
|-------|-------------|
| [RULE-AMMR-025-001](rules/rule-ammr-025-001.md) | Ansvarsbestämning för ensamkommande barn |

### AMMR artikel 26 — Familjemedlem lagligen bosatt

| Regel | Beskrivning |
|-------|-------------|
| [RULE-AMMR-026-001](rules/rule-ammr-026-001.md) | Familjemedlem lagligen bosatt grundar ansvar |

### AMMR artikel 27 — Familjemedlem med pågående ansökan

| Regel | Beskrivning |
|-------|-------------|
| [RULE-AMMR-027-001](rules/rule-ammr-027-001.md) | Familjemedlem med pågående ansökan grundar ansvar |

### AMMR artikel 28 — Familjeförfarande

| Regel | Beskrivning |
|-------|-------------|
| [RULE-AMMR-028-001](rules/rule-ammr-028-001.md) | Familjer ska inte separeras |

### AMMR artikel 29 — Uppehållshandlingar eller viseringar

| Regel | Beskrivning |
|-------|-------------|
| [RULE-AMMR-029-001](rules/rule-ammr-029-001.md) | Stat som utfärdat uppehållshandling eller visering ansvarar |

### AMMR artikel 30 — Examensbevis

| Regel | Beskrivning |
|-------|-------------|
| [RULE-AMMR-030-001](rules/rule-ammr-030-001.md) | Examensbevis grundar ansvar |

### AMMR artikel 31 — Viseringsfri inresa

| Regel | Beskrivning |
|-------|-------------|
| [RULE-AMMR-031-001](rules/rule-ammr-031-001.md) | Viseringsfri inresa grundar ansvar |

### AMMR artikel 32 — Flygplatstransit

| Regel | Beskrivning |
|-------|-------------|
| [RULE-AMMR-032-001](rules/rule-ammr-032-001.md) | Flygplatstransit grundar ansvar |

### AMMR artikel 33 — Inresa

| Regel | Beskrivning |
|-------|-------------|
| [RULE-AMMR-033-001](rules/rule-ammr-033-001.md) | Stat för första irreguljära inresa ansvarar |
| [RULE-AMMR-033-002](rules/rule-ammr-033-002.md) | Ansvar upphör efter 20 månader (irreguljär passage) resp. 12 månader (landsättning) |

### AMMR artikel 35 — Diskretionär bedömning

| Regel | Beskrivning |
|-------|-------------|
| [RULE-AMMR-035-001](rules/rule-ammr-035-001.md) | Medlemsstat får överta prövningsansvaret (suveränitetsklausul) |

### AMMR artikel 34 — Personer i beroendeställning

| Regel | Beskrivning |
|-------|-------------|
| [RULE-AMMR-034-001](rules/rule-ammr-034-001.md) | Sammanförande av beroendesökande |

### AMMR artikel 36 — Ansvarig stats skyldigheter

| Regel | Beskrivning |
|-------|-------------|
| [RULE-AMMR-036-001](rules/rule-ammr-036-001.md) | Skyldighet att överta och återta |

### AMMR artikel 37 — Ansvarets upphörande

| Regel | Beskrivning |
|-------|-------------|
| [RULE-AMMR-037-001](rules/rule-ammr-037-001.md) | Ansvarets upphörande |

### AMMR artikel 38 — Inledande av förfarandet

| Regel | Beskrivning |
|-------|-------------|
| [RULE-AMMR-038-001](rules/rule-ammr-038-001.md) | Skyldighet att inleda förfarandet utan dröjsmål |

### AMMR artikel 39 — Framställan om övertagande

| Regel | Beskrivning |
|-------|-------------|
| [RULE-AMMR-039-001](rules/rule-ammr-039-001.md) | Tidsfrist för framställan (2 månader / 1 månad vid Eurodac-träff) |

### AMMR artikel 40 — Svar på framställan

| Regel | Beskrivning |
|-------|-------------|
| [RULE-AMMR-040-001](rules/rule-ammr-040-001.md) | Tidsfrist för svar och tyst godkännande |

### AMMR artikel 41 — Avisering om återtagande

| Regel | Beskrivning |
|-------|-------------|
| [RULE-AMMR-041-001](rules/rule-ammr-041-001.md) | Avisering inom 2 veckor |

### AMMR artikel 42 — Överföringsbeslut

| Regel | Beskrivning |
|-------|-------------|
| [RULE-AMMR-042-001](rules/rule-ammr-042-001.md) | Överföringsbeslut och information till personen |

### AMMR artikel 43 — Rättsmedel

| Regel | Beskrivning |
|-------|-------------|
| [RULE-AMMR-043-001](rules/rule-ammr-043-001.md) | Rätt till rättsmedel mot överföringsbeslut |

### AMMR artikel 46 — Överföring

| Regel | Beskrivning |
|-------|-------------|
| [RULE-AMMR-046-001](rules/rule-ammr-046-001.md) | Tidsfrist för överföring (6 mån / 18 mån / 4 veckor) |

---

## Tolkningar

| Fil | Frågeställning |
|-----|----------------|
| [first-registration-as-reference-point](interpretations/first-registration-as-reference-point.md) | Vilken tidpunkt är avgörande — registrering eller inresa? |

---

## Öppna frågor

| Fil | Fråga |
|-----|-------|
| [cessation-of-responsibility](open_questions/cessation-of-responsibility.md) | När och hur upphör en stats ansvar? |

---

## Shared capabilities

| Modul | Länk |
|-------|------|
| Children | [shared/children](../../shared/children/README.md) |
| Identity | [shared/identity](../../shared/identity/README.md) |
| Time limits | [shared/time-limits](../../shared/time-limits/README.md) |
