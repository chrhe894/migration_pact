<div align="right">← <a href="README.md">Asylum Procedure</a></div>

# Asylum Procedure Backlog

## Domänöversikt

- [x] README
- [x] Identifiera centrala begrepp
- [x] Identifiera processer
- [x] Identifiera beroenden till Shared

---

## Begrepp

- [x] Determining authority — CON-ASY-001
- [x] Admissibility — CON-ASY-002
- [x] Examination on the merits — CON-ASY-003
- [x] Accelerated procedure — CON-ASY-004
- [x] Subsequent application — CON-ASY-005

---

## Artiklar

- [x] APR artikel 34 — Prövning av ansökningar
- [x] APR artikel 35 — Prövningstidsfrister
- [x] APR artikel 36 — Beslut om ansökningar
- [x] APR artikel 37 — Avslag och återvändande
- [x] APR artikel 38 — Upptagande till prövning
- [x] APR artikel 39 — Beslut i sak
- [x] APR artikel 40 — Uttryckligt återkallande
- [x] APR artikel 41 — Implicit återkallande
- [x] APR artikel 42 — Påskyndat förfarande
- [x] APR artikel 55 — Efterföljande ansökningar
- [x] APR artikel 56 — Undantag från rätt att stanna vid efterföljande ansökningar

---

## Regler

- [x] RULE-APR-034-001 — Individuell, objektiv, opartisk prövning
- [x] RULE-APR-034-002 — Prövning mot flyktingstatus och subsidiärt skydd
- [x] RULE-APR-035-001 — Prövningstidsfrist sex månader
- [x] RULE-APR-035-002 — Förlängning till 15 månader
- [x] RULE-APR-038-001 — Grunder för avvisning
- [x] RULE-APR-042-001 — Grunder för påskyndat förfarande
- [x] RULE-APR-042-002 — Tidsfrist tre månader (påskyndat)
- [x] RULE-APR-055-001 — Förhandsprövning av nya omständigheter
- [x] RULE-APR-055-002 — Avvisning vid avsaknad av nya omständigheter
- [x] RULE-APR-056-001 — Undantag från rätt att stanna

---

## Processer

- [x] PROC-ASY-001 — Examine an application
- [x] PROC-ASY-002 — Subsequent application

---

## Diagram

- [x] Examine an application

---

## Shared dependencies

- [x] interpreters
- [x] identity
- [x] time-limits
- [x] children
- [x] vulnerable-persons
- [ ] interviews — behöver dokumenteras

---

## Tolkningar

- [x] Admissibility vs merits

---

## Öppna frågor

- [x] Implicit withdrawal consequences
- [ ] Förhållandet mellan beslut i sak och gränsförfarande
